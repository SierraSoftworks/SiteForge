...
Route.prototype.outputPath = function(extension) {
	...
	if(extension == '.html')
		return path.resolve(this.forge.options.output, directory == '.' ? filename : directory, (directory != '.' ? filename || 'index' : 'index') + extension);
...
Route.prototype.json = function(object, replacer, spaces) {
	...
	fs.writeFileSync(this.outputPath('.json'), JSON.stringify.apply(this, arguments));
};
...
Route.prototype.render = function(view) {
	...
	compiler.compile(viewPath, this.outputPath(compiler.extension), this.locals);
};
...
Route.prototype.file = function(filepath) {
	...
	fs.writeFileSync(this.outputPath(path.extname(filepath)), fs.readFileSync(path.resolve(process.cwd(), filepath)));
};